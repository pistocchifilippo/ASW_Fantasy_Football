  ...
  <v-main>
    ...
    <v-card class="mx-auto my-12 mobile-login desktop-login">
      <v-form id="registerForm" v-model="isFormValid" name="form" @submit.prevent="handleRegister">
        ...
        <v-card-title flat class="justify-center mobile-title desktop-title" style="font-family:Guardian Headline;font-weight:bold;">
          Fantasy Football
        </v-card-title>

        <v-card-title class="justify-center mobile-subtitle desktop-subtitle" style="font-family:Guardian Headline;font-weight:bold;">
  ...
  export default {
    ...
    computed: {
      passwordConfirmationRule() {
        return () => (this.user.password === this.rPassword) || 'Password must match'
      }
    }, 
    methods: {
      handleRegister: async function(){
        this.errore = "";
        this.registerUser = utils.cryptoMD5(new User(this.user.name,this.user.username, this.user.email, this.user.password));
