    ...
    <v-container>
      ...
      <InfoCard
        v-if="
          ...
          this.infoText != ''
        "
        :color.sync="this.error"
        :text.sync="this.errorMsg"
        class="seventyfive"
      />
      <InfoCard
        v-if="
...
import LineupList from "@/views/lineup/LineupList.vue";
import InfoCard from "@/views/user/InfoCard.vue";
import LineupPlayers from "@/views/lineup/LineupPlayers.vue";
...
export default {
  ...
  beforeMount: async function () {
    ...
    await api.loadAllData(token, this.$router).then((result) => {
  ...
  methods: {
    ...
    save: async function (lineup) {
      ...
      this.profile.lineups[pos] = lineup.map((player) => player._id);
