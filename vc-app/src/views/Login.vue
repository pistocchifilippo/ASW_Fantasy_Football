<template>
  <v-main>
    ...
    <v-card class="mx-auto my-12 mobile-login desktop-login">
      <v-form id="loginForm" v-model="isFormValid" name="login" @submit.prevent="handleLogin" >

        <v-card-title flat class="myfont justify-center mobile-title desktop-title">
          Fantasy Football
        </v-card-title>
        <v-card-title class="myfont justify-center mobile-subtitle desktop-subtitle" >
        ...
        <v-card-actions class="justify-center card-mobile">
          <v-btn :disabled="!isFormValid" type="submit" form="loginForm" class="button-mobile button-desktop" style="font-family:Guardian Headline" color="success">
            Login
          </v-btn>
        </v-card-actions>
        <v-card-title class="error-card justify-center" flat>{{this.errore}}</v-card-title> 
        <v-divider></v-divider>
        <v-card-title flat class=" justify-center" style="font-family:Guardian Headline;font-size:12px">
          If you don't have an account... Join us!
        </v-card-title> 
        <v-card-actions class="justify-center card-mobile"> 
          <router-link class="justify-center card-mobile" to="/signon" style="text-decoration: none;">
            <v-btn center class="justify-center button-mobile" style="font-family:Guardian Headline;color:white;" color="blue">Sign-on</v-btn>
          </router-link>
        </v-card-actions>
      </v-form>
    </v-card>
  ...
  export default {
    ...
    methods: {     
      checkAuth : async function() {
        var ret = await api.checkToken(this.token);
      ...
      handleLogin: async function(){
        this.errore = "";
        this.loginUser = utils.cryptoMD5(new LoginUser(this.user.username, this.user.password));
